 ...
 *
 * @ViewsFilter("wbumenudomain_current_domain_product")
 */
class WbumenudomainCurrentDomainProduct extends BooleanOperator {
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    $this->value_value = t(' Available on current domain product ');
  }
  ...
  public function query() {
    $this->ensureMyTable();
    // $all_table = $this->query->addTable('node__field_domain_all_affiliates',
    // $this->relationship);
    ...
    // $where = "($real_field <> '$current_domain_id' AND ($all_field = 0 OR
    // $all_field IS NULL))";
    // }
    // } else {
    // $where = "($real_field = '$current_domain_id' OR $all_field = 1)";
    // if ($current_domain->isDefault()) {
    // $where = "(($real_field = '$current_domain_id' OR $real_field IS NULL) OR
    // $all_field = 1)";
    // }
    // }
